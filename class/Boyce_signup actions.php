...
namespace XoopsModules\Boyce_signup;
...
class 模組物件
{
    ...
    public static function create($id = '')
    {
        ...
        $op = empty($id) ? "boyce_signup_actions_store" : "boyce_signup_actions_update";
    ...
    public static function store()
    {
        ...
        $sql = "insert into `" . $xoopsDB->prefix("boyce_signup_actions") . "` (
        `欄位1`,
        `欄位2`,
        `欄位3`
        ) values(
    ...
    public static function update($id = '')
    {
        ...
        $sql = "update `" . $xoopsDB->prefix("boyce_signup_actions") . "` set
        `欄位1` = '{$欄位1值}',
        `欄位2` = '{$欄位2值}',
        `欄位3` = '{$欄位3值}'
        where `id` = '$id'";
    ...
    public static function destroy($id = '')
    {
        ...
        $sql = "delete from `" . $xoopsDB->prefix("boyce_signup_actions") . "`
        where `id` = '{$id}'";
        $xoopsDB->queryF($sql) or Utility::web_error($sql, __FILE__, __LINE__);
    }
    ...
    public static function get($id = '')
    {
        ...
        $sql = "select * from `" . $xoopsDB->prefix("boyce_signup_actions") . "`
        where `id` = '{$id}'";
    ...
    public static function get_all($auto_key = false)
    {
        ...
        $sql = "select * from `" . $xoopsDB->prefix("boyce_signup_actions") . "` where 1 ";
        $result = $xoopsDB->query($sql) or Utility::web_error($sql, __FILE__, __LINE__);
        $data_arr = [];
